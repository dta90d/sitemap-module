...
function createSitemapIndexCache(options) {
  const cache = new AsyncCache({
    maxAge: options.cacheTime,
    async load(_, callback) {
      try {
        let sitemaps = typeof options.sitemaps == 'function' ? 
          await options.sitemaps()
          :options.sitemaps
        callback(null, sitemaps)
      } catch (err) {
        /* istanbul ignore next */
        callback(err)
      }
...
module.exports = { createRoutesCache , createSitemapIndexCache}
