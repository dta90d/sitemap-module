...
function registerSitemapIndex(options, globalCache, nuxtInstance, depth = 0) {
  ...
  if(Array.isArray(options.sitemaps)){
    options.sitemaps.forEach((sitemapOptions) => registerSitemaps(sitemapOptions, globalCache, nuxtInstance, depth + 1))
  } else if (typeof options.sitemaps == 'function'){
    try {
      options.sitemaps().then(sitemaps => sitemaps.forEach((sitemapOptions) => registerSitemaps(sitemapOptions, globalCache, nuxtInstance, depth + 1)))
    } catch (error) {
      logger.fatal(error)
    }
  }
